...
void TimerOne::pwm(char pin, int duty, long microseconds)  // expects duty cycle to be 10 bit (1024)
{
  ...
  if(pin == 1 || pin == 9) {
    ...
    TCCR2A |= _BV(COM1A1);                                 // activates the output pin
  }
...
{
  if(pin == 1 || pin == 9)       TCCR2A &= ~_BV(COM1A1);   // clear the bit that enables pwm on PB1
  else if(pin == 2 || pin == 10) TCCR2A &= ~_BV(COM1B1);   // clear the bit that enables pwm on PB2
}
